...
public class ConvidadoFormActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        System.out.println(getIntent()); //Bundle bundle = getIntent().getExtras();
        int handle  = getIntent().getIntExtra(ConvidadoConstants.BundleConstants.CONVIDADO_ID,-1);


        if(handle > 0) {
            Convidado convidado = mConvidadoService.first(handle);
    ...
    private void handleSave() {
        ...
        this.mConvidadoService.save(guestEntity);
